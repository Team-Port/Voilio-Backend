...
public class LikeService {
    ...
    public LikeDto manageLike(User user, LikeCreateRequestDto likeCreateRequestDto) {
        Optional<Like> optionalLike = likeRepository.findByDivisionAndContentIdAndUser(likeCreateRequestDto.getDivision(), likeCreateRequestDto.getContentId(), user);
        Long count = likeRepository.countByDivisionAndAndContentId(likeCreateRequestDto.getDivision(), likeCreateRequestDto.getContentId());
        if (optionalLike.isPresent()) {
            likeRepository.delete(optionalLike.get());
            return new LikeDto(likeCreateRequestDto.getContentId(), likeCreateRequestDto.getDivision(), count - 1, "dislike");
        } else {
